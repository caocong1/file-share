		...
		return new Promise((resolve, reject) => {
			...
			this.ws.onmessage = (event) => {
				try {
					...
					console.log(
						"[WS-DEBUG] 收到WebSocket消息:",
						message.type,
						message.userId || message.from || message.to || "",
					);
					this.handleSignalMessage(message);
				} catch (error) {
					console.error(
						"[WS-DEBUG] 解析WebSocket消息失败:",
						error,
						"Raw data:",
						event.data,
					);
	...
	handleUserOffline(userId) {
		...
		if (connection) {
			...
			const hasActiveTransfer =
				connection.currentTransfer || connection.receivingFile;
