...
class P2PConnection extends EventTarget {
		...
		this.pc.onconnectionstatechange = () => {
			...
			if (this.pc.connectionState === "connected") {
				...
				if (this.receivingFile) {
					console.log(
						"[P2P-DEBUG] 接收文件时连接断开，文件ID:",
						this.receivingFile.id,
					);
					this.dispatchEvent(
						new CustomEvent("filefailed", {
							detail: {
								...
								progress:
									this.receivingFile.receivedSize / this.receivingFile.size,
							},
						}),
					);
				...
				if (this.currentTransfer) {
					console.log(
						"[P2P-DEBUG] 发送文件时连接断开，文件:",
						this.currentTransfer.file.name,
					);
					this.currentTransfer.reject(
						new Error(`连接已断开 (${this.pc.connectionState})`),
					);
					this.currentTransfer = null;
				}
	...
	setupDataChannel() {
		...
		this.dataChannel.onerror = (error) => {
			...
			if (this.currentTransfer) {
				console.log(
					"[P2P-DEBUG] 传输文件时数据通道错误，文件:",
					this.currentTransfer.file.name,
				);
				this.currentTransfer.reject(
					new Error(`数据通道错误: ${error.error?.message || "未知错误"}`),
				);
				this.currentTransfer = null;
			}
	...
	encodeBinaryMessage(message) {
		...
		switch (message.type) {
			case "file-start": {
				...
				const totalSize =
					18 + 8 + 6 + 6 + 2 + nameBytes.length + 2 + typeBytes.length;
	...
	createChunkMessage(id, chunkIndex, buffer) {
		const enableHashVerification =
			config.options.enableHashVerification || false;
		...
		const actualBuffer =
			buffer.byteLength > maxDataSize ? buffer.slice(0, maxDataSize) : buffer;
	...
	async processFileTransfer() {
		...
		try {
				...
				while (retries <= maxRetries) {
					try {
						...
						if (chunkIndex % 200 === 0) {
							// 减少日志噪音
							console.log(
								`[P2P-DEBUG] 发送二进制组合消息，块${chunkIndex}，总大小: ${combinedMessage.byteLength}`,
							);
						}
						...
						if (error.name === "OperationError") {
							if (error.message.includes("send queue is full")) {
								queueFullCount++;
								console.warn(
									`发送队列已满，等待清空... (重试 ${retries}/${maxRetries})`,
								);
								...
								console.warn(
									`发送数据失败，可能是网络问题 (重试 ${retries}/${maxRetries})`,
								);
								await new Promise((resolve) =>
						...
						if (retries > maxRetries) {
							throw new Error(
								`发送失败，已重试 ${maxRetries} 次: ${error.message}`,
							);
						}
					}
				}
			...
			while (
				...
				if (failedChunks.size > 0 && activeTransfers.size < maxConcurrent) {
					...
					const transferPromise = sendChunk(retryChunk)
						...
						.catch((error) => {
							...
							// 如果是队列满错误，降低并发数
							if (
								error.message?.includes("send queue is full") &&
								maxConcurrent > 2
							) {
				...
				// 启动新的传输任务
				while (
					activeTransfers.size < maxConcurrent &&
					currentChunk < totalChunks
				) {
					...
					if (chunkIndex % 100 === 0) {
						// 每100个块输出一次进度
						console.log(
							`[P2P-DEBUG] 🚀 启动块 ${chunkIndex}/${totalChunks} 传输`,
						);
					}
			...
			if (this.userOfflinePending) {
				console.log(
					"[P2P-DEBUG] 文件发送完成，用户已离线，等待保护机制处理关闭",
				);
	...
	sendMessage(message) {
		...
		console.log(
			`[P2P-DEBUG] 发送二进制消息 ${message.type}，大小: ${binaryData.byteLength}字节`,
		);
				...
				if (messageType >= 0x01 && messageType <= 0x06) {
					try {
						if (messageType === 0x01 || messageType === 0x06) {
							...
							if (data.byteLength >= minHeaderSize) {
								...
								if (hasHash) {
									...
									if (calculatedCRC32 !== crc32) {
										console.error(
											`[P2P-DEBUG] ❌ 块${chunkIndex}的CRC32校验失败！期望: ${crc32.toString(16)}, 实际: ${calculatedCRC32.toString(16)}`,
										);
										// 可以在这里请求重传该块
										return;
									} else {
										console.log(
											`[P2P-DEBUG] ✅ 块${chunkIndex}的CRC32校验成功`,
										);
									}
								}

								console.log(
									`[P2P-DEBUG] 收到二进制组合消息，块${chunkIndex}，头部大小${dataOffset}，数据大小${fileData.byteLength}${hasHash ? "，已校验CRC32" : ""}`,
								);
							...
							console.log(
								`[P2P-DEBUG] 收到二进制消息: ${message.type}，大小: ${data.byteLength}字节`,
							);
	...
	handleCombinedChunkMessage(metadata, fileData) {
		...
		console.log(
			`[P2P-DEBUG] 组合消息处理完成，块${metadata.chunkIndex}，进度: ${this.receivingFile.receivedChunks}/${this.receivingFile.totalChunks}`,
		);
	...
	handleFileEnd(message) {
		if (!this.receivingFile || this.receivingFile.id !== message.id) {
			console.log(
				`[P2P-DEBUG] 收到file-end但没有对应的接收文件，ID: ${message.id}`,
			);
			return;
		}
		...
		console.log(
			`[P2P-DEBUG] 当前状态: 已收到${this.receivingFile.receivedChunks}/${this.receivingFile.totalChunks}块`,
		);
		...
		if (this.receivingFile.receivedChunks === this.receivingFile.totalChunks) {
			...
			console.log(
				`[P2P-DEBUG] 总共应接收: ${this.receivingFile.totalChunks} 个块`,
			);
			console.log(
				`[P2P-DEBUG] 实际接收到: ${this.receivingFile.receivedChunks} 个块`,
			);
			console.log(`[P2P-DEBUG] 丢失数量: ${missingChunks.length} 个块`);
			console.log(
				`[P2P-DEBUG] 丢失率: ${((missingChunks.length / this.receivingFile.totalChunks) * 100).toFixed(2)}%`,
			);
			...
			if (missingChunks.length > 0 && missingChunks.length <= 100) {
				console.log(
					`[P2P-DEBUG] 尝试请求重传 ${missingChunks.length} 个缺失块`,
				);
				try {
					this.sendMessage({
						type: "request-retransmit",
				...
				console.log(
					`[P2P-DEBUG] 丢失块过多(${missingChunks.length}个)，可能需要重新发送整个文件`,
				);
			}
		}
	}
	...
	handleRetransmitRequest(message) {
		console.log(
			`[P2P-DEBUG] 收到重传请求，文件ID: ${message.id}，缺失块: ${message.missingChunks.join(", ")}`,
		);
	...
	async retransmitChunks(file, fileId, chunkIndexes) {
		...
		for (const chunkIndex of chunkIndexes) {
			try {
				...
				// 创建组合消息（元数据+数据块）
				const combinedMessage = this.createChunkMessage(
					fileId,
					chunkIndex,
					buffer,
				);
	...
	checkFileCompletion(id) {
		...
		console.log(
			`[P2P-DEBUG] 检查文件完成状态: 已收到${this.receivingFile.receivedChunks}/${this.receivingFile.totalChunks}块`,
		);

		// 检查完成条件：只需要所有块都已接收
		const allChunksReceived =
			this.receivingFile.receivedChunks === this.receivingFile.totalChunks;
	...
	completeFileReception(id) {
		...
		console.log(
			`[P2P-DEBUG] 开始完成文件接收流程，组装${this.receivingFile.totalChunks}个块...`,
		);
		...
		console.log(
			`文件组装完成: ${orderedChunks.length}/${this.receivingFile.totalChunks} 块, 总大小: ${actualFileSize}/${this.receivingFile.size}`,
		);
		...
		if (sizeError > 1024) {
			// 允许1KB的误差
			console.warn(
				`文件大小不匹配: 接收到${file.size}, 期望${this.receivingFile.size}, 误差${sizeError}`,
			);
		}
